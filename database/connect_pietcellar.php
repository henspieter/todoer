...
VALUES ($pid, 'White', 2014, 2015, 2016, 'picture.jpg', 'Bordeaux', 'Nina', 'Frankrijk', 'Bordeaux', 'Pinot Noir')";
    ...
    public function getpreviouswine(){
    ...
    public function removeWine($token) {
        //echo "connect: removing wine";
        $date = date('Y-m-d H:i:s');
        $result = mysqli_query($this->con, "UPDATE Product SET status=0, Date_out='$date'  WHERE token='$token'");
    }

    public function checkWine($token) {
    ...
    public function createWine($wine) {
        $token = $wine['token'];
        $token = mysqli_real_escape_string($this->con, $token);
        $name = $wine['name'];
...
VALUES ($pid, '$type', '$year', '$yearfinish', '$yearstart', '$picture', '$origin', '$gift', '$country', '$region', '$grape','$rating','$price', '$cellar')";
    ...
    function getWines($param=NULL){
        if ($param == NULL){
        $query="select token, status, name, type, count(1) as amount from Product p join Wine w on p.id = w.pid where status = 1  group by type, name";
        } else {
         $query="select token, status, name, type, count(1) as amount from Product p join Wine w on p.id = w.pid where status = 1 and $param  group by type, name";    
        }
       // echo $query;
    ...
    public function editWine($wine){
        ...
        $description = $wine['description'];
        $description = mysqli_real_escape_string($this->con, $description);
        $query = "UPDATE Product SET Name='$name',Description='$description' WHERE token = '$token'";
        if (!mysqli_query($this->con, $query)) {
